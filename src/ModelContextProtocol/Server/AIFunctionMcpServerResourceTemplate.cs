...
/// <summary>Provides an <see cref="AIFunctionMcpServerResourceTemplate"/> that's implemented via an <see cref="AIFunction"/>.</summary>
internal sealed class AIFunctionMcpServerResourceTemplate : McpServerResourceTemplate
{
    private readonly Regex _parser;
    private readonly string[] _variableNames;

    /// <summary>
    /// Creates an <see cref="AIFunctionMcpServerResourceTemplate"/> instance for a method, specified via a <see cref="Delegate"/> instance.
    /// </summary>
    public static new AIFunctionMcpServerResourceTemplate Create(
        Delegate method,
        McpServerResourceTemplateCreateOptions? options)
    {
        Throw.IfNull(method);
    ...
    /// <summary>
    /// Creates an <see cref="McpServerResourceTemplate"/> instance for a method, specified via a <see cref="MethodInfo"/> instance.
    /// </summary>
    public static new AIFunctionMcpServerResourceTemplate Create(
        MethodInfo method,
        object? target,
    ...
    private static AIFunctionFactoryOptions CreateAIFunctionFactoryOptions(
        MethodInfo method, McpServerResourceTemplateCreateOptions? options) =>
        new()
        {
    ...
    public static new AIFunctionMcpServerResourceTemplate Create(AIFunction function, McpServerResourceTemplateCreateOptions? options)
    {
        ...
        return new AIFunctionMcpServerResourceTemplate(function, resource);
    }

    private static McpServerResourceTemplateCreateOptions DeriveOptions(MemberInfo member, McpServerResourceTemplateCreateOptions? options)
    {
        McpServerResourceTemplateCreateOptions newOptions = options?.Clone() ?? new();

        if (member.GetCustomAttribute<McpServerResourceAttribute>() is { } resourceAttr)
        {
            newOptions.UriTemplate ??= resourceAttr.Uri;
    ...
    private AIFunctionMcpServerResourceTemplate(AIFunction function, ResourceTemplate resourceTemplate)
    {
        AIFunction = function;
        ProtocolResourceTemplate = resourceTemplate;

        _parser = UriTemplate.CreateParser(resourceTemplate.UriTemplate);
        _variableNames = _parser.GetGroupNames().Where(n => n != "0").ToArray();
    }
        ...
        RequestContext<ReadResourceRequestParams> request, CancellationToken cancellationToken = default)
    {
        ...
        if (_parser.Match(request.Params.Uri) is not { Success: true } match)
        {
            return null;
        }
        ...
        foreach (string varName in _variableNames)
        {
