...
{
    /// <summary>
    /// Gets or sets the legacy Model Context Protocol version that the client wants to use with
    /// the <c>initialize</c> handshake.
    /// </summary>
    /// <remarks>
    /// <para>
    /// Protocol version is specified using a date-based versioning scheme in the format "YYYY-MM-DD".
    /// The client and server must agree on a legacy protocol version to communicate successfully.
    /// </para>
    /// <para>
    /// During initialization, the server will check if it supports this requested legacy version. Protocol
