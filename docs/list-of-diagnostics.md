...
| Diagnostic ID | Description |
| :------------ | :---------- |
| `MCPEXP001` | Experimental APIs tied to MCP specification features, including [HTTP/2 Streamable HTTP over stdio](xref:transports#experimental-http2-streamable-http-over-stdio). Reuse this ID for newly introduced experimental spec features, and add feature-specific messages/URLs in `Experimentals`. |
