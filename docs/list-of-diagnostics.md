...
| `MCPEXP002` | Experimental extensibility points in the C# SDK implementation that are unrelated to the MCP specification itself, including subclassing `McpClient`/`McpServer` (see [#1363](https://github.com/modelcontextprotocol/csharp-sdk/pull/1363)), custom request handlers, alternate handlers and filters, outgoing request interception, routing metadata, and `RunSessionHandler`, which may be removed or change signatures in a future release (consider using `ConfigureSessionOptions` instead). |
