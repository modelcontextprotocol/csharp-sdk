...
public class ResumabilityTests : LoggedTest
{
    ...
    public void StreamableHttpServerTransport_HasRetryIntervalProperty()
    {
        ...
        transport.RetryInterval = 5000;

        // Assert
        Assert.Equal(5000, transport.RetryInterval);
    }
    ...
    // Note: Tests for SseWriter.Complete() and StreamableHttpServerTransport.CloseStandaloneSseStream()
    // are internal implementation details. The CloseSseStream functionality is tested through integration
    // tests in ResumabilityIntegrationTests.cs which test the complete end-to-end behavior.
